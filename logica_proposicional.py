...
def iguais(a, b) -> bool:
    ...
    if a[0] == '~':
        return iguais(a[1], b[1])

    # comutatividade
    if a[0] in ('&', '|'):
        return (
            (iguais(a[1], b[1]) and iguais(a[2], b[2]))
            or
            (iguais(a[1], b[2]) and iguais(a[2], b[1]))
        )
...
def simplificar(formula: str) -> List[Tuple[str, str]]:
    ...
    arvore_atual = elim_impl(arvore)
    registrar("Eliminação de → e ↔  [ (A→B) ≡ (¬A∨B) ]", arvore_atual)
    ...
    arvore_atual = intern_neg(arvore_atual)
    registrar("De Morgan + dupla negação  [ ¬(A∧B)≡¬A∨¬B  |  ¬¬A≡A ]", arvore_atual)
    ...
    def reduzir(no):
        ...
        # 3. Absorção
        if op == '|' and d[0] == '&' and (iguais(e, d[1]) or iguais(e, d[2])): return e, "Absorção  [ A ∨ (A∧B) ≡ A ]"
        if op == '|' and e[0] == '&' and (iguais(d, e[1]) or iguais(d, e[2])): return d, "Absorção  [ (A∧B) ∨ A ≡ A ]"
        if op == '&' and d[0] == '|' and (iguais(e, d[1]) or iguais(e, d[2])): return e, "Absorção  [ A ∧ (A∨B) ≡ A ]"
        if op == '&' and e[0] == '|' and (iguais(d, e[1]) or iguais(d, e[2])): return d, "Absorção  [ (A∨B) ∧ A ≡ A ]"
        
        # 4. Distributiva
        if op == '|' and d[0] == '&':
            return ('&', ('|', e, d[1]), ('|', e, d[2])), "Distributiva  [ A ∨ (B∧C) ≡ (A∨B) ∧ (A∨C) ]"
        if op == '|' and e[0] == '&':
            return ('&', ('|', d, e[1]), ('|', d, e[2])), "Distributiva  [ (B∧C) ∨ A ≡ (A∨B) ∧ (A∨C) ]"
        if op == '&' and d[0] == '|':
            return ('|', ('&', e, d[1]), ('&', e, d[2])), "Distributiva  [ A ∧ (B∨C) ≡ (A∧B) ∨ (A∧C) ]"
        if op == '&' and e[0] == '|':
            return ('|', ('&', d, e[1]), ('&', d, e[2])), "Distributiva  [ (B∨C) ∧ A ≡ (A∧B) ∨ (A∧C) ]"
    ...
    # Continuamos rodando a redução até que ela retorne nenhuma 'lei' aplicada (None)
    while True:
        nova_arvore, lei_aplicada = reduzir(arvore_atual)
        if lei_aplicada is None:
            break  # Nenhuma regra pôde ser aplicada, a árvore está totalmente simplificada
        arvore_atual = nova_arvore
        registrar(lei_aplicada, arvore_atual)
